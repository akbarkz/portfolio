"""
Migration script to transfer content from the frontend translations.ts file to MongoDB.

This script stores translations in the i18next format:
{
  "en": { "translation": { ... } },
  "kz": { "translation": { ... } }
}
...
# Translation data extracted from translations.ts
# This matches the i18next format exactly
translations_data = {
    'en': {
        'translation': {
            'projects': 'Projects',
    ...
    'kz': {
        'translation': {
            ...
            'main': {
                ...
                'text': {
                    'p1': 'Бұл вебсайт менің бағдарламалау дүниеме шолу мүмкіндігін қамтамасыз етеді. Бұл - менің қызықты жобалар топтамасы, соның ішінде менің немен қызығатыным және не істей алатыным туралы мағлұмат. Бұ жерде жаңа идеяларды жүзеген асырған жеке жобаларым мен басқалармен бірлесіп қабілеттерімді қолданған басқа да әріптестіктерім туралы ақпарат таба аласыз.',
                    'p2': 'Байланысу, қызметтесу немесе жай бағдарламалау инженерлік ісіне қатысты қызықты заттарды талқылаймын десеңіз, менімен хабарласуыңызды сұраймын. Кіріп шыққаныңыз үшін рахмет және көргеніңіз ұнайды деп үміттенемін.',
            ...
            'about': {
                'header': 'Өзім жайлы',
                'p1': 'Менің есімім Әбдірахман Акбар, мен Қазақстанның Алматы қаласында тұратын бағадарламалар инженерімін 🇰🇿. Мен күшті сандық тәжірибелерді (бағдарламаларды) құрастыруды жақсы көремін. Вэб саласында құрастыру шеберлігімді жылдар бойы ұштап келемін, әсіресе фронтэнд бағытында.',
                'p2': 'Бағдаламалар инженерлік дүниесі әрдайым өзгеруде, бірақ маған ең көп фронтэнд саласы ұнайды. Мен - React, Vue және Javascript пен Typescript шеберімін. Бірақ бұл менің шеберліктерімнің толық тізімі емес, - оған қоса менің бэкенд құрастырудан және CI/CD дүниесінен де біраз хабарым бар.',
                ...
                'p4': 'Жұмыстарымды шолу сізге ұнайды деп үміттенемін!',
            },
            'education': {
                'header': 'Білім',
                'thesis': {
...
def migrate_translations():
    """
    Migrate translations to MongoDB in i18next format.
    Each language gets its own document with the 'translation' key.
    """
    for lang_code, lang_data in translations_data.items():
        # Insert or update the translation document for this language
        db.translations.update_one(
            {'language': lang_code},
            {'$set': {'language': lang_code, 'data': lang_data}},
            upsert=True
        )
        print(f"✓ Migrated translations for language: {lang_code}")
...
def verify_migration():
    ...
    for translation in db.translations.find({}):
        ...
        keys_count = len(translation.get('data', {}).get('translation', {}).keys())
        print(f"  - {lang}: {keys_count} top-level keys")

if __name__ == '__main__':
    print("Starting translation migration to MongoDB...")
    print("=" * 50)
    migrate_translations()
    verify_migration()
    print("=" * 50)
    print("\n✅ Migration completed!")
    print("\nYou can now fetch translations from the API:")
    print("  - GET /api/translations/en")
    print("  - GET /api/translations/kz")
